...
template <typename T>
SupernodalSolver<T>::~SupernodalSolver() {
#ifdef GC_HAVE_SUITESPARSE
  if (internals->cMat != nullptr) {
    cholmod_l_free_sparse(&internals->cMat, internals->context);
    internals->cMat = nullptr;
...
template <typename T>
SupernodalSolver<T>::SupernodalSolver(SparseMatrix<T>& mat, bool useGPU)
    : LinearSolver<T>(mat), internals(new PSDSolverInternals<T>()) {
  ...
  if(useGPU)
    ((cholmod_common*)internals->context)->useGPU = 1;
...
template <typename T>
bool SupernodalSolver<T>::factorize(SparseMatrix<T>& mat) {
...
template <typename T>
Vector<T> SupernodalSolver<T>::solve(const Vector<T>& rhs) {
  Vector<T> out;
  solve(out, rhs);
  return out;
}
...
template <typename T>
void SupernodalSolver<T>::solve(Vector<T>& x, const Vector<T>& rhs) {
...
template class SupernodalSolver<double>;
template class SupernodalSolver<float>;
template class SupernodalSolver<std::complex<double>>;
